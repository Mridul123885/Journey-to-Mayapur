...
tab, built additively — **zero lines of Gau Seva's or the Journey chapters'
existing code were touched.**

**Important honesty note on tech stack**: this app was never built in
Phaser 3/TypeScript — every game in it, including this new one, is plain
HTML/CSS/JS in the single `index.html`, wrapped by Capacitor. The new game
uses the same falling-collectible engine (rAF motion + `pointerdown` hit
detection + a collected-guard flag) already proven in Gau Seva, written as
its own separate `mp*`-prefixed functions so nothing overlaps.

**What's in it:**
- **5 story-driven stages**, each opening with a real illustration and a
  short story panel using your actual "Story Illustrations" artwork and
  captions verbatim (Skip Story / Start Game buttons, same as the Journey
  chapters), then a falling-collectible stage themed to that scene, ending
  with a stars/score results screen.
- **Infinite Sankirtan Journey**, unlocked after Stage 5 — the same engine
  running with no target score, continuously escalating difficulty, and a
  persisted high score with a "🏆 New high score!" callout.
- **10 achievement badges** using your actual badge medallion artwork —
  Village Hero, Persistence, Blessed, Careful Steps, Untouchable, Doubt
  Master, Boost Master, Full Mercy, Transformation, Enlightened Being.
  (The badge *images* baked in visible text with some AI-garbled spelling —
  e.g. "BERSISTENCE," "PULLMERLY" — so the medallion artwork is used, but
  the actual names/descriptions shown in-app are typed cleanly in code, not
  read off the image.)
- **Dedicated background music**, using the real track you uploaded this
  time — auto-plays on entering any screen of this game, loops, and stops
  completely on exit to Home, the hub, or any other game. It never touches
  Gau Seva's or the Journey chapters' music, and they never touch it.
- **Save system** via the same Preferences-backed storage as everything
  else: unlocked stages, best stars per stage, Endless Mode unlock, high
  score, coins, and earned badges all persist and reload correctly.

**Honest simplifications, so nothing here is oversold:**
- Your reference sheets are character turnarounds (walk/run/jump/idle/chant
  poses), not seamless flipbook animation frames — the game shows one
  representative pose per state (idle/walk/celebrate) rather than true
  frame-by-frame walk-cycle animation. Real animation blending across those
  36 frames per character is a further pass, not done here.
- "Mercy Hearts" and "Golden Stars" collectibles have no dedicated art in
  what was uploaded, so they render as ❤️ and ⭐ rather than invented
  artwork.
- The "Buttons & UI," "Achievement Badges" (other copies), and "Level
  Icons" sheets you sent were a different, more polished art style than
  the character/background sheets used for the actual game — mixing both
  styles in one scene would look inconsistent, so only the pieces that
  matched (badge medallions, the Endless Mode sign) were used; the
  colorful icon/button set from those sheets wasn't pulled in.
- No Jagai/Madhai boss encounter or dedicated "attack" mechanic — the
  story beats (including the attack on Nityananda Prabhu) are told through
  the illustrations and captions, while the stage gameplay itself stays on
  the same proven falling-collectible mechanic throughout, for reliability.

## 📖 Chapter stories now use the exact provided text, verbatim

Every chapter's story text is now the exact paragraph supplied for that
chapter — no paraphrasing, no rewriting. Titles, lessons, and inspiration
lines are unchanged from the previous pass.

## 🔒 Debug tools are now truly build-gated, not just hidden by a toggle

The Test & Debug panel (Send Test Reminder button + on-screen debug log)
is invisible by default in the HTML itself, and is only revealed by a real
native check — a small Capacitor plugin (`DebugInfoPlugin.java`) that
reports Android's own `BuildConfig.DEBUG` flag. That flag is set by the
Gradle build variant at compile time (`assembleDebug` vs.
`assembleRelease`/`bundleRelease`), not by any runtime setting, so it
cannot be accidentally left on. If the plugin can't be reached for any
reason, or the app is running outside the native build entirely, the panel
stays hidden — the failure mode is "hidden," never "visible."

**Practical effect on the existing GitHub Actions workflow**: `assembleRelease` and `bundleRelease` (the outputs already produced by
`.github/workflows/android-build.yml`) will never show the debug panel —
that's the Play Store-bound release build. Only a debug-variant build
(`assembleDebug`, not currently in the workflow) would show it, for local
testing purposes.

## 🎵 One dedicated Story Music track for every Story screen

Story screens now use their own separate audio system, fully independent
from gameplay music:

- **Plays automatically** the moment any Story screen opens, looping for as
  long as it's open.
- **Stops completely** the instant the player finishes reading, taps Skip,
  taps Start Game, opens the Story Library, or leaves the Story screen for
  any other reason — every one of those paths calls the same
  `leaveStoryScreen()` cleanup before anything else happens.
- **Backgrounding**: if the app is minimized while a Story is open, the
  track pauses immediately (via `visibilitychange`, plus the Capacitor
  `App` plugin's `appStateChange` for reliable native detection). On
  return, it resumes only if that Story screen is still open — if the
  story was already skipped or exited while the app was backgrounded,
  playback does not resume.
- **Never overlaps gameplay music**: Story music is fully stopped and its
  `Audio` element disposed of (`src` cleared, reference dropped) *before*
  `startLevel()` runs and gameplay music begins — the two can never sound
  at once.

**On the audio file itself**: no dedicated Story Music track was uploaded
in this pass, so rather than invent one, the system currently reuses the
existing chapter background theme as `STORY_MUSIC_SRC`. That's one clearly
marked constant near the top of the story-music code — swap it for a real
uploaded track and the whole story-music behavior (play/stop/pause/resume/
dispose) works unchanged.
